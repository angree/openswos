...
**Why it regressed:** the PREVIOUS Android build (S33h, task #236) WORKED on the user's Realme 7 Pro, so the break came from THIS session's shared-code changes. Prime suspects (bisect these first, they're this session's Main.cs changes): (1) **RenderScale const→`static readonly` + static-init** (Main.cs ~7226-7237: `HiResScale`/`FastRender`/`MatchScale`/`MenuScale`). `FastRender` must be FALSE on Android — `ComputeFastRender()` claims to exclude Android via `DisplayServer.GetName()=="headless"` / `OS.HasFeature("mobile")` / `OS.GetName()` Android/iOS, but a static field initializer reading `OS`/`DisplayServer` at TYPE-INIT (before the scene tree/singletons are fully ready on Android) can THROW and kill the app — verify the try/catch actually wraps every OS/DisplayServer call in `ComputeFastRender` and that a throw defaults to hi-res. (2) i18n: a `Loc.Tr` on a key whose .tsv row is malformed, or the `data/i18n/*.tsv` / `data/*.csv` export include_filter change. (3) the commentator/audio wiring. **Bisect method:** the S33h APK worked — `git diff` this session's Main.cs static-init/render-mode block vs before; if suspect (1), gate the whole fast-render/ContentSizeForState path behind an even earlier "is this a handheld Linux build" guard and make static init bulletproof on Android. Rebuild APK: `I:\GITHUB\W_OPEN_SWOS\.tools\godot\Godot_v4.6.2-stable_mono_win64_console.exe --headless --path I:\GITHUB\W_OPEN_SWOS\game --export-release "Android" I:\GITHUB\W_OPEN_SWOS\build\android\OpenSWOS.apk` (Godot HANGS on exit after writing the APK — wrap in `timeout 420`; the APK is complete once its size is stable across a few seconds and it starts with "PK"). User updates via Total Commander LAN from that exact path.

## >>> R36S — SOLVED & PLAYABLE (WestonPack — do NOT re-chase FRT / arm32 / X11)
OpenSWOS runs and a MATCH plays **smoothly** on the R36S. The earlier "crash" was: the bare Linux export needs X11/Wayland, but ArkOS/stock R36S renders only to the KMS/DRM framebuffer (no display server). **FRT is Godot-3-only and GDScript-only — dead end.** The working path is **WestonPack** (PortMaster runtime = headless Weston/Wayland+Xwayland+GL compositor): ship our OWN self-contained **arm64** build and run it inside WestonPack. Full details + all launcher gotchas are in memory `r36s-godot-runs-via-westonpack` (a.k.a. file `r36s-debug-read-the-log.md`). Key facts: run our own binary (NOT PortMaster's GDScript godot runtime — that has no .NET); binary sits directly in `ports/OpenSWOS/` so BINDIR=GAMEDIR; **drop `--main-pack`** (official template rejects path overrides — Godot auto-loads `OpenSWOS.pck` next to the binary); the device has TWO PortMasters, so capture the abs `weston_pkg_0.2.squashfs` path from the SD one (`/roms/ports/PortMaster/libs`) BEFORE sourcing control.txt (it clobbers `$controlfolder`) and re-pin after; strip CRLF→LF; `OPENSWOS_HANDHELD=1` in the launcher auto-enables SMOOTHING + fast render. Project already `renderer/rendering_method="gl_compatibility"` + ETC2/ASTC (WestonPack prereqs met). **Full public HOWTO written this session: `docs/porting/godot4-csharp-on-portmaster-handhelds.md` — standalone open-source guide for anyone shipping a Godot-4 C# game to an RK3326 handheld (likely a public first for Godot-4-C# on PortMaster).** **Deploy paths:** ready arm64 build `I:\GITHUB\W_OPEN_SWOS\build\linux-arm64\` (OpenSWOS.arm64 + OpenSWOS.pck + data_OpenSwos_linuxbsd_arm64\); launcher/package `I:\GITHUB\W_OPEN_SWOS\build\r36s-weston\OpenSWOS.sh`; on the SD card (mounts as `G:\`): copy `OpenSWOS.pck`+`data.../OpenSwos.dll` into `G:\ports\OpenSWOS\`, launcher to `G:\ports\OpenSWOS.sh`, runtime `weston_pkg_0.2.squashfs` into `G:\ports\PortMaster\libs\`. After any PC-side write to `G:\`, `Write-VolumeCache -DriveLetter G` (or the launcher's `sync`) before the user pulls the card, else the write is lost. The R36S arm64 build (dll built 22:13, includes THIS session's changes + i18n) is READY and waiting for the user to reinsert the card ("karta").

## SESSION 34 CONTINUED (2026-07-18 evening) — R36S WestonPack + big S34 batch
- **R36S WestonPack**: solved as above; menu+music+match all working, match runs smoothly. Likely a public first for Godot-4-C# on a PortMaster handheld.
- **Fast render mode** (Main.cs): handheld/640x480 build renders the MATCH at `MatchScale=2` (native 768x544) instead of the 3x supersample (1152x816) → ~55% fewer shaded pixels, fixes match slowdown + harsh downscale. MENUS stay hi-res 1152x816 (per-state `ContentSizeForState()`; menu bitmap charset must stay crisp). `RenderScale`→`MatchScale` (static readonly, FastRender?2:3). Escape hatch env `OPENSWOS_FAST=0`. Desktop/Android unchanged (FastRender false).
- **Stamina** (Sim/Port/PlayerEnergy.cs): (a) **match-length normalization** — per-tick drain divisor ×(totalMatchSeconds/360 ref) so a 3-min and 10-min match end EQUALLY tired (`SetMatchLength`, called Main.cs match setup); (b) **half-time recovery** — `RecoverAtHalfTime()` recovers a FLAT 40% of each player's LOST energy (NOT scaled by stamina — user rejected double-dipping), called from GameLoop.FirstHalfJustEnded; (c) keeper: movement effort 20→10 (tires less from moving), catch drain 1-3%→2-4%.
- **AI shot spin FIXED** (SwosVm/Memory.cs + Sim/Port/AiBrain.cs): the 4 AI aftertouch tables (`AI_leftSpinTable` etc.) were declared but NEVER seeded (read as 0) → AI shot flat, no curve/loft. Seeded them with swos.asm:246182 values (-1,-2,-3 / 1,2,3 / -32,32 / 0,-999,4) AND fixed the paired steer misport in AiBrain.cs (`newD1=(d1+delta)&7`, was `d1Adj+delta`). MUST ship together (half-fix inverts). 
- **Penalty-save FIXED** (SwosVm/Memory.cs `penaltyDiveRow` ~1881): the table had 15 `7,0` pairs instead of 16 (58 vs 60 bytes) → shifted every element, so element 21 read 0 not 2 and the keeper could NEVER cleanly claim a penalty save (always deflected loose → attacker taps in). Added the missing pair. Verified vs original-amiga-swos.asm:35574.
- **i18n wrapping (~120 strings)**: the whole in-match layer in Main.cs had ZERO Loc.Tr (pause/overlays/result/bench/first-run) + Competition/Career had zero. Wrapped all via 2 parallel implementers (key prefixes match./overlay./result./bench./pos./opt. for Main; menu./nav./comp./career./staff./nation. for menu/comp). Persisted+logic-matched strings (Fixture.Stage, trophies, manager title, comp names) translated at DISPLAY time only (new `Competition/CompLoc.cs` TrStage/TrSpecialty/Ordinal) — stored/compared values stay English so saves+logic unbroken. 127 new keys translated to all 19 languages (game/data/i18n/*.tsv now 561 rows each, 0 non-ASCII, 0 dup keys, validated). DEFERRED: 86 country names (big proper-noun batch, still English).
- **COMMENTATOR ON/OFF (#239)**: OPTIONS audio page toggle (`opt.commentator`), gates CommentaryEngine via `MatchAudio.CommentatorEnabled` (persisted `_commentator`, default ON; only affects PC sound — Amiga has no spoken commentary).
- **CLAUDE.md**: added "WRITE IN CLEAR, UNAMBIGUOUS LANGUAGE" section (plain YES/NO status + full path + timestamp; no hedging) — user directive; also in memory `clear-unambiguous-language`.
- Build state: `dotnet build` 0/0 across all the above; R36S arm64 re-exported (dll 22:13, i18n in pck); Android APK exported (22:24) but NOT WORKING (see top). Card OUT of PC at compaction time.

## SESSION 34 (2026-07-18) — team picker, tuning, ball control, i18n (20 languages), v0.3 release

## SESSION 34 (2026-07-18) — team picker, tuning, ball control, i18n (20 languages)
- **Team picker = tiles + drill-down**: FRIENDLY / LOCAL-MP / TEAM SETUP now open a
  CONTINENT→COUNTRY→CLUB drill-down of 3-column TILES (filled/bordered, centred text),
  one in-place picker (SwapListPicker + KeepOpen) so levels never bleed through
  (fixed the "miksuje kraje i kluby" kasza). ESC climbs a level. MenuClient.cs.
- **Clock = authentic 54** (GameTime.kPcTicksPerGameSecond 70→54, swos.asm:97838): the
  genuine PC clock; menu length = WHOLE MATCH (SecondsPerHalf = total/2). Half-time ~21%
  sooner. Pitch fix: PitchFile.cs cell-0 is tile #0 (not transparent) — SWCPICH6 cut
  stripes gone.
- **Fatigue tuning**: stamina 7-vs-1 durability = 1.67 ((8+s)*24), reserve ×0.9. Being
  TACKLED drains 1-5% of current energy (PlayerEnergy.DrainOnTackle, sim-Rng). KEEPER
  tires from catches: 1-3% per catch (DrainOnKeeperCatch at gs!=3 gate) + save-skill −1/−2
  at ≤50%/≤20% (KeeperSkillPenalty → RunShotAtGoal). Scoreboard leftover on menu FIXED
  (SetResultPanelVisible(false) in the menu branch of UpdateUi).
- **FAITHFUL BALL CONTROL** (PlayerActions.FaithfulBallControl, default TRUE): the control
  mechanic was fully ported (kBallSpeedDeltaWhenControlled run-away, dseg_17E276 turn-break,
  duels) but MASKED by a per-tick outfield ball pin. Un-pinned (keeper pin kept) → weak
  dribblers lose the ball. NOTE: AI-vs-AI scoring dropped in smoke — NEEDS PLAYTEST; if the
  ball runs away from GOOD players too, flip the flag false + do chase-AI work.
- **i18n — 20 LANGUAGES**: game/scripts/Menu/Loc.cs (Tr(key,english) + LoadAll from
  res://data/i18n/<code>.tsv), LANGUAGE selector on OPTIONS pg1 (Accent PAGE + gap),
  persisted. Master DB docs/i18n/strings.tsv (394 strings + context). 19 translations
  (EN identity) generated by subagents, ASCII-only, validated (394 rows, 0 non-ASCII).
  ~440 call sites wired through Loc.Tr across MenuClient.cs / MenuClient.Career.cs /
  Main.MenuHost.cs (home, options, friendly, ALL career screens). Remaining: footer control
  hints (no DB keys yet), kSortModes/kEntrantModes/kManagerTitles static arrays, competition
  stage names, LengthLabel interpolation.
- New menu background (game/data/openswos_bg.png). Verified: build 0/0, boot OK,
  --competition-test PASSED, smoke 30k OK.
...
- **STAMINA durability (final tune, user spec)**: weak vs strong energy must NOT diverge
  ~2x. Durability divisor `(14 + stamina) * 18`: stamina-7 = 378, stamina-1 = 270 →
  **378/270 = 140:100 (1.40x)** exactly (user: "7 v 1 to wytrzymalosc 140 do 100"), and
  whole durability ×1.23 vs the prior tune (everyone lasts longer). Start-energy penalty
  `(7-s)*24` (negligible ~3.5% gap). PlayerEnergy.cs kStaminaFloor=14, kDivisorScale=18.
- **Clock length = WHOLE MATCH (2x-too-long bug FIXED)**: the menu length preset was being
  fed in as seconds-PER-HALF, so a "3 MIN" match ran two 3-min halves = 6 min real (user:
  "mecz 3 min jest 2x dluzszy niz na amidze i pc"). Original SWOS length = TOTAL match. Now
  `SecondsPerHalf = TotalMatchSeconds/2` (Main.cs:355); default 3 MIN → 90 s/half →
  `timeDelta = 2700/90 = 30`, EXACTLY the port's kGameLenSecondsTable[0]=30 (3-min entry).
  Labels now "N MIN REAL (WHOLE MATCH)".
- **RANDOM pitch default** (`_pitchRandom`, rolled per match) — career was always pitch 1.
- **PITCH RENDER BUG FIXED (cut stripes)**: PitchFile.cs treated map-cell value 0 as
  "transparent/no-draw", but 0 is a valid ref to tile #0 (a real light-grass tile).
  SWCPICH1-5 never use it, but SWCPICH6 (training pitch, now reachable via RANDOM) builds
  every mowed light stripe from tile-#0 columns → each stripe was cut in half with flat
  untextured green (user "dziwne pasy przeciete"). Fix: deleted the `if(offset==0)continue`
  special case in PitchFile.cs:82 (shared into game via Compile Include). Pitches 1-5
  provably unchanged; pitch 6 now renders continuous stripes. Verified by re-rendering
  SWCPICH6 (pitch tool).
